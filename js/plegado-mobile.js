(() => {
    ...
    function renderResults() {
        ...
        if (!state.filteredRecords.length) {
            // Puede ser un registro recien agregado al Sheet: re-sincroniza cuando
            // el usuario deja de escribir (no en cada tecla).
            scheduleAutoRefreshOnMiss();
    ...
    async function handleProcesar() {
        ...
        try {
            // Una sola llamada batch en vez de una peticion por fila: registra
            // mas rapido y evita saturar el Apps Script con N POST paralelos.
            const changes = {
                plegado_turno: turno,
            ...
            const updates = selectedIds
                .filter((recordId) => findRecordById(recordId))
                .map((recordId) => ({ id_registro: recordId, changes }));
    ...
    // Consultar el Sheet en cada tecla congelaba la interfaz en el celular:
    // la re-sincronizacion por "sin resultados" espera a que el usuario deje
    // de escribir. Si el Sheet ya se consulto hace poco (p. ej. por una pausa
    // a mitad de la OP-PTDA), la verificacion se reprograma para cuando se
    // cumpla el intervalo minimo en vez de descartarse: la consulta completa
    // nunca se pierde, y mientras siga "sin resultados" en pantalla se vuelve
    // a verificar sola cada AUTO_REFRESH_MIN_INTERVAL_MS.
    const AUTO_REFRESH_MISS_DEBOUNCE_MS = 1200;
    let autoRefreshMissTimer = null;

    function scheduleAutoRefreshOnMiss() {
        if (autoRefreshMissTimer) clearTimeout(autoRefreshMissTimer);
        const sinceLastRefresh = Date.now() - lastAutoRefreshAt;
        const delay = Math.max(AUTO_REFRESH_MISS_DEBOUNCE_MS, AUTO_REFRESH_MIN_INTERVAL_MS - sinceLastRefresh);
        autoRefreshMissTimer = window.setTimeout(() => {
            autoRefreshMissTimer = null;
            if (document.visibilityState === 'hidden') return;
            if (!state.currentQuery) return;
            if (filterByExactOpPartida(state.currentQuery).length) return;
            requestAutoRefresh();
        }, delay);
    }
